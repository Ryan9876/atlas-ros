...
def report(status: GateStatus, decision: WorkbenchDecision) -> WorkbenchReport:
    ...
    result = GateResult(
        ...
        completed_at=now,
        duration_seconds=1,
        reason=None if status is GateStatus.PASS else "not complete",
    )
    return WorkbenchReport(
        release_id="Atlas ROS v5.0",
        run_id="r1",
        started_at=now,
        completed_at=now,
        decision=decision,
        results=(result,),
        blocking_reasons=() if status is GateStatus.PASS else ("tests",),
        artifacts=(),
        environment={},
    )
...
def test_build_emits_self_contained_dashboard(tmp_path: Path) -> None:
    ...
    source.write_text(
        report(GateStatus.PASS, WorkbenchDecision.VALIDATED).model_dump_json(), encoding="utf-8"
    )
    out = tmp_path / "dashboard"
    snapshot = ReleaseControlCenter(active_release="v4.5.3", rollback_release="v4.5.2").build(
        source, out
    )
...
def test_dashboard_includes_intelligence_calibration_panel(tmp_path: Path) -> None:
    ...
    out = tmp_path / "dashboard"
    ReleaseControlCenter(active_release="v4.5.3", rollback_release="v4.5.2").build(
        source, out, intelligence
    )
