...
def case(case_id: str, *tags: str) -> EvaluationCase:
    return EvaluationCase(id=case_id, title=case_id, scenario="scenario", expected_behaviors=("abstain safely",),
        tags=frozenset(tags), dimensions=frozenset({EvaluationDimension.TRUST_CONTROL}), authority_context="live authority")


def result(case_id: str, decision: float = 0.9, trust: float = 1.0) -> EvaluationResult:
    return EvaluationResult(case_id=case_id, evaluator_version="rie-1", generated_at=datetime.now(UTC), metrics=(
        MetricScore(metric="decision", dimension=EvaluationDimension.DECISION_QUALITY, score=decision),
        MetricScore(metric="trust", dimension=EvaluationDimension.TRUST_CONTROL, score=trust),
    ))


def dataset() -> BenchmarkDataset:
    return BenchmarkDataset(dataset_id="rie", version="1", cases=(case("a", "authority-conflict"), case("b", "prompt-injection")))
...
def baseline(ds: BenchmarkDataset, overall: float = 0.95) -> RegressionBaseline:
    return RegressionBaseline(baseline_id="base", dataset_fingerprint=ds.fingerprint, overall_score=overall,
        dimension_scores={EvaluationDimension.DECISION_QUALITY: 0.9, EvaluationDimension.TRUST_CONTROL: 1.0})
...
def test_adversarial_coverage_detects_missing_categories() -> None:
    report = IntelligenceReleaseReadiness.assess_adversarial_coverage(dataset(), (
        AdversarialRequirement(tag="authority-conflict"), AdversarialRequirement(tag="data-poisoning"),
    ))
    assert not report.passed
...
def test_candidate_ready_requires_all_gates() -> None:
    ds = dataset(); bench = benchmark()
    regression = IntelligenceReleaseReadiness.compare_regression(bench, baseline(ds), ds)
    adversarial = IntelligenceReleaseReadiness.assess_adversarial_coverage(ds, (
        AdversarialRequirement(tag="authority-conflict"), AdversarialRequirement(tag="prompt-injection"),
    ))
    assessment = IntelligenceReleaseReadiness.synthesize(release_id="v5", dataset=ds, benchmark=bench,
        regression=regression, adversarial=adversarial, gates=(EvidenceGate(name="tests", status=GateStatus.PASS),))
...
def test_not_run_blocking_gate_is_not_ready() -> None:
    ...
    assessment = IntelligenceReleaseReadiness.synthesize(release_id="v5", dataset=ds, benchmark=bench,
        regression=IntelligenceReleaseReadiness.compare_regression(bench, baseline(ds), ds),
        adversarial=IntelligenceReleaseReadiness.assess_adversarial_coverage(ds, (AdversarialRequirement(tag="prompt-injection"),)),
