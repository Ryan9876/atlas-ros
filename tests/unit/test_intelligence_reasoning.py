...
def setup(tmp_path: Path):
    ...
    ctx = ContextSnapshot(
        ...
        created_at=NOW,
        active_objective="Choose implementation approach",
        decision_horizon="current milestone",
        evidence_refs=(ev1.ref(), ev2.ref()),
    )
...
def request(ev1, ev2, ctx, margin=0.05):
    return ReasoningRequest(
        ...
        context_ref=ctx.ref(),
        criteria=(
            DecisionCriterion(name="benefit", weight=2),
            DecisionCriterion(name="risk", weight=1, direction=CriterionDirection.MINIMIZE),
        ),
        options=(
            OptionAssessment(
                option="phased",
                scores={"benefit": 0.9, "risk": 0.2},
            ...
            OptionAssessment(
                option="big bang",
                scores={"benefit": 0.7, "risk": 0.8},
                expected_benefit="Fast completion",
                expected_risk="High change risk",
                evidence_refs=(ev1.ref(),),
            ),
        ),
...
def test_engine_selects_evidence_supported_option(tmp_path: Path):
    ...
    assert (
        outcome.trace.ranked_options[0].adjusted_score
        > outcome.trace.ranked_options[1].adjusted_score
    )
...
def test_engine_abstains_when_margin_is_insufficient(tmp_path: Path):
    ...
    outcome = GovernedReasoningEngine(store).evaluate(
        request(ev1, ev2, ctx, margin=0.9), created_at=NOW
    )
...
def test_rejected_or_low_confidence_evidence_blocks_recommendation(tmp_path: Path):
    store = SQLiteIntelligenceRecordStore(tmp_path / "records.db")
    store.initialize()
    weak = evidence(4, confidence=0.2, status=ValidationStatus.REJECTED)
    ctx = ContextSnapshot(
        record_id=UUID("00000000-0000-4000-9000-000000000005"),
        created_at=NOW,
        active_objective="test",
        decision_horizon="now",
        evidence_refs=(weak.ref(),),
    )
    store.append_many((weak, ctx))
    req = ReasoningRequest(
        objective="test",
        context_ref=ctx.ref(),
        criteria=(DecisionCriterion(name="value", weight=1),),
        options=(
            OptionAssessment(
                option="a",
                scores={"value": 0.9},
...
def test_request_rejects_missing_scores_and_duplicates(tmp_path: Path):
    ...
    with pytest.raises(ValidationError, match="score every criterion"):
        ReasoningRequest(
            objective="x",
            context_ref=ctx.ref(),
            criteria=(DecisionCriterion(name="a", weight=1), DecisionCriterion(name="b", weight=1)),
            options=(
                OptionAssessment(
                    option="one",
                    scores={"a": 0.5},
    ...
    with pytest.raises(ValidationError, match="option names must be unique"):
        ReasoningRequest(
            objective="x",
            context_ref=ctx.ref(),
            criteria=(DecisionCriterion(name="a", weight=1),),
            options=(
                OptionAssessment(
                    option="same",
                    scores={"a": 0.5},
