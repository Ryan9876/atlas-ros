...
def completion(outcome: str, done: str, evidence: str = "verified record") -> CompletionDimensionsV1:
...
def test_exact_duplicate_requires_completion_equivalence() -> None:
    ...
    decision = decide_relationship(
        ...
        related_records=(RelatedRecordV1(record_id="a1", title="Define a standard naming convention for ANX customer devices", completion=proposed),),
...
def test_paraphrase_is_duplicate_only_when_all_dimensions_match() -> None:
    ...
    decision = decide_relationship(
        ...
        proposed_completion=proposed,
        related_records=(RelatedRecordV1(record_id="a1", title="Define a standard naming convention", completion=proposed),),
        familiarity=familiar(),
        consequence=ConsequenceAssessmentV1(),
    )
...
def test_anx_central_management_is_preserved_and_clarified() -> None:
    records = (
        RelatedRecordV1(record_id="naming", title="ANX device naming convention", completion=completion("standard naming", "approved naming standard")),
        RelatedRecordV1(record_id="snmp", title="Install SNMP discovery probes in LIT and ALL", completion=completion("CMDB discovery", "customer devices discovered in CMDB")),
    )
    decision = decide_relationship(
...
def test_same_domain_different_done_is_not_duplicate() -> None:
    proposed = completion("centralized configuration", "devices controlled through approved platform")
    related = RelatedRecordV1(record_id="snmp", title="Discover devices", completion=completion("CMDB discovery", "devices discovered in CMDB"))
    decision = decide_relationship(
        capture="centrally manage ANX customer devices",
        proposed_completion=proposed,
...
def test_low_history_and_high_consequence_increase_clarification() -> None:
    decision = decide_relationship(
        ...
        consequence=ConsequenceAssessmentV1(security=True, production=True, reversible=False),
    )
    assert decision.evidence_level is EvidenceLevel.PARTIAL
    assert decision.clarification_status is ClarificationStatus.REQUIRED


def test_confirmed_corrections_raise_contextual_evidence_without_overriding_current_intent() -> None:
    evidence = tuple(
        IntentEvidenceV1(evidence_id=f"e{i}", context_key="anx", interpretation="separate outcome", confirmed=True, corrected_by_user=i < 2)
        for i in range(4)
    )
    decision = decide_relationship(
        capture="new ANX lifecycle outcome",
        proposed_completion=completion("lifecycle management", "lifecycle controls operational"),
        related_records=(),
        familiarity=familiar(),
        consequence=ConsequenceAssessmentV1(),
        evidence=evidence,
...
def test_stale_or_contradictory_evidence_does_not_strengthen_inference() -> None:
    evidence = (
        IntentEvidenceV1(evidence_id="stale", context_key="x", interpretation="duplicate", confirmed=True, stale=True),
        IntentEvidenceV1(evidence_id="contradiction", context_key="x", interpretation="separate", confirmed=True, contradictory=True),
    )
    decision = decide_relationship(
        ...
        familiarity=familiar(0.1),
        consequence=ConsequenceAssessmentV1(vendor=True, external_commitment=True),
        evidence=evidence,
    )
    assert decision.evidence_level is EvidenceLevel.PARTIAL
    assert decision.clarification_status is ClarificationStatus.REQUIRED
