...
Execution Reconciliation keeps Notion reporting current by reconciling governed Todoist execution changes back to ROS. It is attended and review-first.
...
Conflicts are written to ROS Operations when configured. Reconciliation never overwrites Notion-authoritative management fields and never treats a webhook payload as canonical state.
