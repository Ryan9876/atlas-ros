...
Introduce `TaskUpdateLifecycleNormalizer` before the existing lifecycle interpreter. It uses deterministic, narrow evidence rules and produces an existing `AtlasCommandV1`. It recognizes explicit delegation/ownership, waiting-on, blocked, complete, update, or no actionable transition. Qualified delegation requires a unique responsible person, expected outcome, and completion criteria. Delegate due date and Ryan follow-up checkpoint are separate fields.

The normalizer cannot authorize execution or call providers. Its output flows through `CommandLifecycleService`, the existing canonical planner, attended authorization, adapters, idempotent transaction journal, readback, and reconciliation. Generic update behavior is unchanged; a no-action update does not create a follow-up.
...
- Material ambiguity blocks provider planning.
...
- The Notion schema change is additive and activation-controlled.
