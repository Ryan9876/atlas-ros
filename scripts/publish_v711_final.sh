...
rm -rf final-artifact final-publication final-evidence release-readback \
  rollback-v710 rollback-v701 rollback-v650 rollback-v620 \
  verify-v711 verify-v710 verify-v701 verify-v650 verify-v620
...
assert identity['release_version'] == '7.1.1'
...
assert identity['active_production_release'] == '7.1.0'
...
checks = {
    'atlas_ros-7.1.1.tar.gz': os.environ['EXPECTED_SOURCE_SHA256'],
    'atlas_ros-7.1.1-py3-none-any.whl': os.environ['EXPECTED_WHEEL_SHA256'],
...
cp "$artifact_zip" final-publication/atlas-ros-v7.1.1-final-package.zip
cp final-artifact/atlas_ros-7.1.1.tar.gz final-publication/
cp final-artifact/atlas_ros-7.1.1-py3-none-any.whl final-publication/
...
cp final-artifact/initialization-performance.json final-publication/
...
git show "${FINAL_SOURCE_COMMIT}:release/RELEASE_MANIFEST_V711.md" > final-publication/RELEASE_MANIFEST_V711.md

test "$(sha256sum final-publication/RELEASE_MANIFEST_V711.md | awk '{print $1}')" = "$EXPECTED_MANIFEST_RAW_SHA256"
manifest_canonical="$(python - <<'PY'
from pathlib import Path
from atlas_ros.kernel.digests import sha256_digest
print(sha256_digest(Path('final-publication/RELEASE_MANIFEST_V711.md').read_text(encoding='utf-8')))
PY
)"
...
export SOURCE_COMMIT FINAL_ARTIFACT_ID FINAL_ARTIFACT_DIGEST DECISION_URL \
  PACKAGE_REVIEW_URL PREPUBLICATION_REVIEW_URL RELEASE_TAG ROLLBACK_TAG PUBLISH
python - <<'PY'
import json
import os
from pathlib import Path
...
authorization = {
    ...
    'package_review': 'V4V-74',
    'package_review_url': os.environ['PACKAGE_REVIEW_URL'],
    'prepublication_review': 'V4V-75',
...
notes = '''# Atlas ROS v7.1.1

Atlas ROS v7.1.1 is the authorized immutable corrective release for faster GitHub-first Quick Initialization. It preserves fail-closed authority, live mutable System State and Integration Inventory reads, attended execution, and the exact required integration set of GitHub, Notion, and Todoist.

Publication does not itself activate production authority. Atlas ROS v7.1.0 remains Active until independent publication readback and matching GitHub and Notion authority activation complete.

Immediate rollback after activation: Atlas ROS v7.1.0. Historical rollbacks retained: v7.0.1, v6.5.0, and v6.2.0. Google Drive remains optional, non-authoritative, and outside initialization.
'''
(root / 'RELEASE_NOTES.md').write_text(notes, encoding='utf-8')
manifest = f'''# Atlas ROS v7.1.1 Production Publication Manifest
...
python -m venv verify-v711
verify-v711/bin/python -m pip install --disable-pip-version-check final-publication/atlas_ros-7.1.1-py3-none-any.whl
verify-v711/bin/python -c "import atlas_ros; assert atlas_ros.__version__ == '7.1.1'"
stage v711-clean-install-passed

for pair in "v7.1.0 7.1.0 rollback-v710 verify-v710" "v7.0.1 7.0.1 rollback-v701 verify-v701" "v6.5.0 6.5.0 rollback-v650 verify-v650" "v6.2.0 6.2.0 rollback-v620 verify-v620"; do
  set -- $pair
...
PY
  ...
  test "$(sha256sum release-readback/atlas-ros-v7.1.1-final-package.zip | awk '{print $1}')" = "${FINAL_ARTIFACT_DIGEST#sha256:}"
  test "$(sha256sum release-readback/atlas_ros-7.1.1.tar.gz | awk '{print $1}')" = "$EXPECTED_SOURCE_SHA256"
  test "$(sha256sum release-readback/atlas_ros-7.1.1-py3-none-any.whl | awk '{print $1}')" = "$EXPECTED_WHEEL_SHA256"
  test "$(sha256sum release-readback/RELEASE_MANIFEST_V711.md | awk '{print $1}')" = "$EXPECTED_MANIFEST_RAW_SHA256"
  python -m venv release-readback/clean-install
  release-readback/clean-install/bin/python -m pip install --disable-pip-version-check release-readback/atlas_ros-7.1.1-py3-none-any.whl
  release-readback/clean-install/bin/python -c "import atlas_ros; assert atlas_ros.__version__ == '7.1.1'"
