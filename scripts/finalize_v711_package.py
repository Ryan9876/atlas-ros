#!/usr/bin/env python3
"""Assemble the non-publishing Atlas ROS v7.1.1 final package."""
...
VERSION = "7.1.1"
MANIFEST_PATH = Path("release/RELEASE_MANIFEST_V711.md")
V710_COMMIT = "0711b045f34f5ab7b03f7a61bc80653e0d815463"
...
def assemble(
    *,
    ...
    if "Package version: `7.1.1`" not in manifest_text:
        raise ValueError("immutable manifest does not identify v7.1.1")
    if f"Atlas ROS v7.1.0 at `{V710_COMMIT}`" not in manifest_text:
        raise ValueError("immutable manifest does not identify the exact v7.1.0 rollback")
    ...
    sbom = {
        ...
        "creationInfo": {
            ...
            "creators": ["Tool: Atlas ROS v7.1.1 final-package assembler"],
        },
    ...
    authority = compile_authority(
        AuthorityCompilationSpec(
            active=ActiveReleaseSpec(
                ...
                release_url="https://github.com/Ryan9876/atlas-ros/releases/tag/v7.1.1",
                source_sha256=source_sha,
                wheel_sha256=wheel_sha,
            ),
            rollback=RollbackReleaseSpec(
                version="7.1.0",
                immutable_commit=V710_COMMIT,
                tag="v7.1.0",
            ...
            last_promotion_transaction_id=("v711-promotion-pending-" + source_commit[:12]),
    ...
    final_identity = {
        ...
        "immediate_rollback_after_promotion": "7.1.0",
