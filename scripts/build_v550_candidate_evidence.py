...
RELEASE_NAME = "Atlas ROS v5.5.0"
EVIDENCE_PREFIX = "V550"
BASE_AUTHORITY = "Atlas ROS v5.4.0"
...
def build_evidence(
    *,
    ...
    if not version.startswith("5.5."):
        raise ValueError(f"expected an Atlas ROS v5.5 package version, got {version}")
...
This compatibility candidate completes roadmap Wave 4 Execution Planning and Task Economy:
Execution Candidate, Projection Decision, Existing Representation Index, Execution Plan V2,
deterministic extraction, the complete Task Projection Test, progressive horizons, layered
duplicate analysis, task budgets, anti-bloat enforcement, explainability, W03A compatibility,
and restoration evidence. It does not authorize execution or perform provider writes.
...
{execution["case_count"]} Execution Planning benchmark cases with zero provider writes and zero
planner authorizations.
"""
    (output_dir / "RELEASE_MANIFEST_V550_CANDIDATE.md").write_text(manifest, encoding="utf-8")
    blockers = [
    ...
    status_path = output_dir / "V550_PROMOTION_PREPARATION_STATUS.json"
    _write_json(status_path, status)
    (output_dir / "V550_PROMOTION_PREPARATION_STATUS.sha256").write_text(
...
"""
    (output_dir / "V550_PROMOTION_PREPARATION_REPORT.md").write_text(report, encoding="utf-8")
...
promotion.
"""
    (output_dir / "V550_RELEASE_VALIDATION_WORKBENCH_REPORT.md").write_text(
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Build Atlas ROS v5.5.0 candidate evidence")
