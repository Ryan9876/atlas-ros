...
RELEASE_NAME = "Atlas ROS v5.6.0"
EVIDENCE_PREFIX = "V560"
BASE_AUTHORITY = "Atlas ROS v5.5.0"
...
def build_evidence(
    *,
    ...
    if not version.startswith("5.6."):
        raise ValueError(f"expected an Atlas ROS v5.6 package version, got {version}")
...
This compatibility candidate completes roadmap Wave 5 Execution Orchestration and Provider
Separation: exact scoped execution authorization, deterministic provider-neutral commands,
transaction state and journal contracts, bounded retries, idempotent replay, uncertain-apply
readback, partial-failure recovery, fail-closed receipts, Todoist and Notion adapter boundaries,
W03 compatibility, and restoration evidence. It performs no live provider writes during
validation and does not grant unattended authority.
...
provider writes and zero false-success receipts.
"""
    (output_dir / "RELEASE_MANIFEST_V560_CANDIDATE.md").write_text(manifest, encoding="utf-8")
    blockers = [
    ...
    status_path = output_dir / "V560_PROMOTION_PREPARATION_STATUS.json"
    _write_json(status_path, status)
    (output_dir / "V560_PROMOTION_PREPARATION_STATUS.sha256").write_text(
...
"""
    (output_dir / "V560_PROMOTION_PREPARATION_REPORT.md").write_text(report, encoding="utf-8")
...
"""
    (output_dir / "V560_RELEASE_VALIDATION_WORKBENCH_REPORT.md").write_text(
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Build Atlas ROS v5.6.0 candidate evidence")
